...
def migrate_legacy_risk_flags(state: dict[str, Any]) -> bool:
    ...
    if outcome in {"triggered", "inconclusive"}:
        state.setdefault("findings", []).append({"finding_id": f"finding:csp-address:migrated:{uuid4().hex}", "schema_version": "finding/v1", "category": "csp_address", "assessment_id": assessment_id, "check_id": "csp_address", "title": "Company service provider address", "summary": summary, "subject": {"entity_type": "company", "name": assessment["company_name"] or "Customer"}, "confidence": {"level": confidence, "rationale": "Migrated from the retired CSP risk-flag record.", "limitations": limitations}, "severity": {"level": "not_applicable", "rationale": "CSP address detection is an address-service indicator; it does not independently assess the severity of financial-crime risk."}, "potential_impact_risk": "A registered address associated with a company service provider can obscure the entity's operating presence.", "recommended_action_rfi": {"internal_actions": ["Review the company’s operating presence and address rationale."], "rfi": [{"request": "Provide evidence of the company’s operating address and business presence.", "reason": "To establish whether the registered address reflects an operating presence.", "priority": "medium"}]}, "source": {"producer_type": "tool", "producer_name": "csp_address_assessment", "run_id": run_id, "created_at": created_at}, "relevant_evidence_ids": [evidence_id], "migration": {"method": "legacy_risk_flags_migration", "legacy_finding_id": flag.get("finding_id"), "limitations": limitations}})
    return True
