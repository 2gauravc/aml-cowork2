...
def evaluate_digital_footprint(company_name: str, *, jurisdiction: str | None=None, registration_number: str | None=None, known_domain: str | None=None, registered_address: str | None=None) -> dict[str, Any]:
    ...
    try: parsed=json.loads(traced_openai_client(OpenAI()).responses.create(model=DEFAULT_MODEL,input=[{"role":"user","content":[{"type":"input_text","text":prompt}]}],text={"format":{"type":"json_schema","name":"digital_footprint_assessment","schema":schema,"strict":True}}).output_text)
