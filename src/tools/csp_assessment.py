...
def assess_csp_address(state: dict[str, Any], *, address: str | None = None, company_name: str | None = None) -> dict[str, list[dict[str, Any]]]:
    ...
    assessment = {"assessment_id": assessment_id, "assessment_type": "csp_address", "schema_version": "csp_address_assessment/v1", "tool": "csp_address_assessment", "run_id": run_id, "created_at": created_at, "outcome": outcome, "summary": summary, "registered_address": address, "company_name": company_name, "confidence": (result.get("assessment") or {}).get("confidence", "low" if outcome != "not_triggered" else "medium"), "definition": {"skill_path": result.get("definition_path"), "definition_version": result.get("definition_version")}, "source_evidence_ids": [evidence_id], "source_urls": [item.get("url") for item in result.get("sources", []) if item.get("url")], "result": result}
    findings: list[dict[str, Any]] = []
    if outcome in {"triggered", "inconclusive"}:
        severity = ((result.get("finding_policy") or {}).get("severity") or {"level": "not_applicable", "rationale": "CSP address detection is an address-service indicator; it does not independently assess the severity of financial-crime risk."})
        findings.append({"finding_id": f"finding:csp-address:{uuid4().hex}", "schema_version": "finding/v1", "category": "csp_address", "assessment_id": assessment_id, "check_id": "csp_address", "title": "Company service provider address", "summary": summary, "subject": {"entity_type": "company", "name": company_name or "Customer"}, "confidence": {"level": assessment["confidence"], "rationale": "Derived from the CSP-address assessment.", "limitations": [result.get("reason")] if result.get("reason") else []}, "severity": severity, "potential_impact_risk": "A registered address associated with a company service provider can obscure the entity's operating presence.", "recommended_action_rfi": {"internal_actions": ["Review the company’s operating presence and address rationale."], "rfi": [{"request": "Provide evidence of the company’s operating address and business presence.", "reason": "To establish whether the registered address reflects an operating presence.", "priority": "medium"}]}, "source": {"producer_type": "tool", "producer_name": "csp_address_assessment", "run_id": run_id, "created_at": created_at}, "relevant_evidence_ids": [evidence_id]})
