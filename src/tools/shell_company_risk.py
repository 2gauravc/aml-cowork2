...
def _classify(inputs: dict[str, Any], factors: list[dict[str, Any]]) -> dict[str, dict[str, str]]:
    ...
    try:
        response = OpenAI().responses.create(model=DEFAULT_MODEL, input=[{"role": "user", "content": [{"type": "input_text", "text": prompt}]}], text={"format": {"type": "json_schema", "name": "shell_company_risk", "schema": schema, "strict": True}})
