...
def test_completed_pipeline_persists_the_full_graph_state() -> None:
    ...
    saved = {
        "saved_at": "2026-07-27T00:00:01+00:00",
        "identity": {"customer_name": "UBIZENSE LIMITED"},
    }
...
def test_loading_legacy_state_replaces_its_model_rating_with_the_rule_based_rating() -> (
    None
):
    graph_state = {
        "findings": [],
        "assessments": [
            {
                "assessment_id": "adverse",
                "assessment_type": "adverse_news",
                "outcome": "completed_no_material_findings",
                "created_at": "2026-01-01T00:00:00Z",
                "source_evidence_ids": [],
            },
            {
                "assessment_id": "shell",
                "assessment_type": "shell_company_risk",
                "outcome": "not_triggered",
                "created_at": "2026-01-01T00:00:00Z",
                "source_evidence_ids": [],
            },
            {
                "assessment_id": "industry",
                "assessment_type": "other_risk_factors",
                "factor_id": "high_risk_industry",
                "outcome": "not_triggered",
                "created_at": "2026-01-01T00:00:00Z",
                "source_evidence_ids": [],
            },
            {
                "assessment_id": "aml",
                "assessment_type": "other_risk_factors",
                "factor_id": "high_aml_risk_jurisdiction_link",
                "outcome": "not_triggered",
                "created_at": "2026-01-01T00:00:00Z",
                "source_evidence_ids": [],
            },
            {
                "assessment_id": "tax",
                "assessment_type": "other_risk_factors",
                "factor_id": "high_tax_risk_jurisdiction_link",
                "outcome": "not_triggered",
                "created_at": "2026-01-01T00:00:00Z",
                "source_evidence_ids": [],
            },
            {
                ...
                "rating": "standalone_high",
                "summary": "Legacy model result",
            },
        ],
    }
    ...
    ratings = [
        item
        for item in graph_state["assessments"]
        if item["assessment_type"] == "risk_rating"
    ]
...
def _legacy_csp_state(evaluation: str | None) -> dict:
    return {
        ...
        "risk_flags": [
            {
                "finding_id": "csp_address:legacy",
                "category": "csp_address",
                "evaluation": evaluation,
                "severity": "medium",
                "description": "Legacy CSP result.",
                "evidence": {
                    "assessment": {
                        "is_csp": evaluation,
                        "confidence": "medium",
                    ...
                    "sources": [{"url": "https://example.test/csp"}],
                },
            }
        ],
    }
...
def test_legacy_csp_outcomes_become_canonical_records() -> None:
    for evaluation, outcome, findings in (
        ("no", "not_triggered", 0),
        ("yes", "triggered", 1),
        ...
        assert migrate_legacy_csp_address(state) is True
        assert state["assessments"][-1]["schema_version"] == "csp_address_assessment/v2"
...
def test_migrated_csp_view_is_stable_and_second_migration_is_a_noop() -> None:
    ...
    assert view["detailed"]["assessment_tags"] == [
        {"label": "Confidence", "value": "medium", "tone": "confidence"},
        {"label": "Severity", "value": "not_applicable", "tone": "severity"},
    ]
    assert migrate_legacy_csp_address(state) is False


def test_legacy_ownership_only_flags_are_dropped_and_migration_is_idempotent() -> None:
    state = {
        "risk_flags": [{"category": "ownership", "evaluation": "yes"}],
        "evidence": [],
        "assessments": [],
        "findings": [],
    }
...
def test_malformed_legacy_csp_is_visible_as_an_inconclusive_migration_limitation() -> (
    None
):
    ...
    assert not list(
        Draft202012Validator(load_finding_schema()).iter_errors(state["findings"][-1])
    )
...
def test_loading_a_migrated_legacy_snapshot_persists_it() -> None:
    ...
    with patch("src.backend.app.get_completed_state", return_value=snapshot), patch(
        "src.backend.app.save_completed_state",
        return_value={
            "saved_at": "2026-01-02T00:00:00Z",
            "identity": snapshot["identity"],
        },
    ) as persist, patch("src.backend.app.asyncio.to_thread", to_thread):
...
def test_legacy_adverse_news_artifacts_are_normalized_idempotently() -> None:
    state = {
        "evidence": [
            {
                "evidence_id": "evidence:adverse:1",
        ...
        "findings": [
            {
                "finding_id": "finding:adverse:1",
                "category": "adverse_news",
                "subject": {
                    "entity_type": "ultimate_beneficial_owner",
                    "name": "Alex Chen",
                },
                "relevant_evidence_ids": ["evidence:adverse:1"],
                "adverse_news": {"screening_coverage": {"queries": ["Alex Chen"]}},
    ...
    assert (
        state["evidence"][0]["data"]["web_search_evidence"]["schema_version"]
        == "web_search_evidence/v1"
    )
...
def test_adverse_news_view_hides_storage_shape_from_the_ui() -> None:
    state = {
        "assessments": [
            {
                "assessment_id": "assessment:adverse",
                "assessment_type": "adverse_news",
                "outcome": "completed_no_material_findings",
            }
        ],
...
def test_presentation_label_edit_changes_the_compiled_view() -> None:
    ...
    assert (
        adverse_news_view(state, definition)["summary"]["metrics"][0]["label"]
        == "Subjects screened"
    )
...
def test_invalid_presentation_binding_is_rejected() -> None:
    ...
    with pytest.raises(AdverseNewsViewError, match="does not resolve"):
        adverse_news_view(
            {"assessments": [], "findings": [], "evidence": []}, definition
        )
...
def test_loading_legacy_adverse_news_persists_the_normalized_snapshot() -> None:
        ...
        "findings": [
            {
                "finding_id": "finding:adverse:1",
                "category": "adverse_news",
                "subject": {"entity_type": "company", "name": "Example Ltd"},
                "relevant_evidence_ids": ["evidence:adverse:1"],
            }
        ],
    }
    ...
    with patch("src.backend.app.get_completed_state", return_value=snapshot), patch(
        ...
        response = asyncio.run(
            load_completed_cdd_state(
                CDDStateLookupRequest(
                    session_id="adverse-migration-test",
                    customer_name="Example Ltd",
                    jurisdiction="GB",
                )
            )
    ...
    assert (
        response["cdd_state"]["tool_views"]["adverse_news"]["schema_version"]
        == "tool_view/v1"
    )
    assert response["cdd_state"]["findings"][0]["assessment_id"]
    assert (
        response["cdd_state"]["tool_views"]["adverse_news"]["detailed"]["findings"][0][
            "tags"
        ][2]["value"]
        == "Not retained"
    )
...
def test_loading_legacy_digital_footprint_persists_a_stable_view() -> None:
    state = {
        "evidence": [
            {
                "evidence_id": "evidence:digital:1",
        ...
        "findings": [
            {
                "finding_id": "finding:digital:1",
                "category": "digital_footprint",
                "subject": {"entity_type": "company", "name": "Example Ltd"},
                "relevant_evidence_ids": ["evidence:digital:1"],
            }
        ],
    }
    snapshot = {
    ...
    with patch("src.backend.app.get_completed_state", return_value=snapshot), patch(
        ...
        response = asyncio.run(
            load_completed_cdd_state(
                CDDStateLookupRequest(
                    session_id="digital-migration-test",
                    customer_name="Example Ltd",
                    jurisdiction="GB",
                )
            )
    ...
    assert (
        response["cdd_state"]["tool_views"]["digital_footprint"]["schema_version"]
        == "tool_view/v1"
    )
    assert (
        response["cdd_state"]["findings"][0]["migration"]["method"]
...
def test_resumed_pipeline_syncs_completion_and_persists_the_completed_state() -> None:
    ...
    session = {
        ...
        "graph_thread_id": "thread-1",
        "graph_state": {
            "documents": resumed_state["documents"],
            "case_status": {"cdd_generation": "in_progress"},
        },
    }
    saved = {
    ...
    with patch(
        "src.backend.app.resume_cdd_agent_state", return_value=resumed_state
    ) as resume, patch(
        "src.backend.app.save_completed_state", return_value=saved
    ) as persist:
...
def test_document_action_queues_a_resume_without_waiting_for_the_full_pipeline() -> (
    None
):
    session = {
        "session_id": "session-1",
